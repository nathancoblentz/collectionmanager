...
class UpdateItemWindow(FormWindow):
    def __init__(self, master=None, refresh_callback=None):
        ...
        self.item_map = {}  # Maps display name to ItemID
        ...
        for key, label in self.fields.items():
            self.entries[key] = self.labeled_entry(label)
    ...
    def load_items(self):        
        ...
        if user["Role"] == "Admin":
            cursor.execute("SELECT ItemID, ItemName FROM Item WHERE Status = 'Active'")
        else:
            cursor.execute("SELECT ItemID, ItemName FROM Item WHERE Status = 'Active' AND Username = ?", (user["Username"],))
        ...
        if not items:
            ...
            self.item_dropdown.set("No items available")
            self.item_dropdown.config(state="disabled")
        else:
            self.item_map = {item["ItemName"]: item["ItemID"] for item in items}
            self.item_dropdown['values'] = list(self.item_map.keys())
        ...
        if collections:
            self.collection_dropdown['values'] = [collection["CollectionName"] for collection in collections]
            self.collection_dropdown.current(0)  # Default to first collection

        # Load sources for dropdown
        cursor = conn.cursor()
        cursor.execute("SELECT SourceID, BusinessName FROM Source")
        sources = cursor.fetchall()
        cursor.close()

        if sources:
            self.source_dropdown['values'] = [source["BusinessName"] for source in sources]
            self.source_dropdown.current(0)  # Default to first source
    ...
    def on_item_selected(self, event=None):
        ...
        conn = connect()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM Item WHERE ItemID = ?", (item_id,))
        ...
        if item:
            for key in self.fields:
                ...
                self.entries[key].insert(0, str(value or ""))

            # Set selected collection and source based on item data
            cursor = conn.cursor()
            cursor.execute("SELECT CollectionName FROM Collection WHERE CollectionID = ?", (item["collectionID"],))
            collection = cursor.fetchone()
            cursor.close()

            if collection:
                self.collection_dropdown.set(collection["CollectionName"])

            cursor = conn.cursor()
            cursor.execute("SELECT BusinessName FROM Source WHERE SourceID = ?", (item["sourceID"],))
            source = cursor.fetchone()
            cursor.close()

            if source:
                self.source_dropdown.set(source["BusinessName"])
    ...
    def submit(self):
        ...
        # Get the collection ID based on the selected collection name
        cursor.execute("SELECT CollectionID FROM Collection WHERE CollectionName = ?", (collection_name,))
        collection = cursor.fetchone()
        cursor.close()

        if not collection:
            messagebox.showerror("Error", "Invalid collection selected.")
            return

        # Get the source ID based on the selected source name
        cursor = conn.cursor()
        cursor.execute("SELECT SourceID FROM Source WHERE BusinessName = ?", (source_name,))
        source = cursor.fetchone()
        cursor.close()
        ...
        # Update the item with the selected collection and source
        data["collectionID"] = collection["CollectionID"]
        data["sourceID"] = source["SourceID"]

        try:
            item = Item.get_by_identifier(self.selected_item_id)
            if item:
                item.update(**data)
                message = f"Item '{data['itemName']}' updated successfully."
                messagebox.showinfo("Success", message)
                log(message)
                if self.refresh_callback:
                    self.refresh_callback()
...
### COLLECTION ###
        ...
        self.collectionname_entry = self.labeled_entry("Collection Name", key="collectionname")
...
class UpdateSourceWindow(FormWindow):
    def __init__(self, master=None, refresh_callback=None):
        ...
        # Dropdown - ONLY use BusinessName now
        self.source_dropdown, self.source_var = self.labeled_dropdown(
            "Select Source:",
            query="SELECT BusinessName FROM Source"  # 👈 No SourceID here
        )
        self.source_dropdown.bind("<<ComboboxSelected>>", self.prefill_fields)

        # Form entries
        self.entries = {key: self.labeled_entry(label) for key, label in self.fields.items()}

        # Buttons
        self.add_buttons(submit_text="Update Source", cancel_text="Cancel")

        # Initialize dropdown data
        self.load_dropdown_data(self.source_dropdown, "SELECT BusinessName FROM Source", [])

    def load_dropdown_data(self, dropdown, query, params, map_name=None):
        try:
            conn = connect()  # Assuming you have a `connect()` function for the DB
            cur = conn.cursor()
            ...
            # Use only the first column (BusinessName) for the dropdown
            values = [row[0] for row in rows]
            dropdown['values'] = values
            dropdown.set("Select a source")
    ...
    def prefill_fields(self, event=None):
        selected_name = self.source_var.get().strip()
        if not selected_name:
            return

        source = Source.get_by_identifier(selected_name)  # 👈 Use BusinessName directly
        if not source:
            messagebox.showerror("Error", f"Source '{selected_name}' not found.")
            return

        # Fill each entry with the source's existing data
    ...
    def submit(self):
        """
        Handle the form submission (update the source or perform any action).
        """
        selected_name = self.source_var.get().strip()
        ...
        # Now you can handle the update logic here
        messagebox.showinfo("Submit", f"Source '{data['businessName']}' has been submitted for update.")
        # Example of calling refresh callback after successful update (if applicable)
        if self.refresh_callback:
            self.refresh_callback()
    ...
    def cancel(self):
        """
        Handle the cancel action (simply close the window).
        """
        self.destroy()
