...
class DeleteUserWindow(tk.Toplevel):
    ...
    def submit(self):
        ...
        try:
            ...
            if user:
                user.delete()
                messagebox.showinfo("Success", f"User '{selected_user}' has been deleted.")
                self.load_users()  # 🔁 Refresh dropdown
                if self.refresh_callback:
                    self.refresh_callback()
            else:
...
class AddCollectionWindow(FormWindow):
    def __init__(self, master=None, refresh_callback=None):
        ...
        self.collectiontype_dropdown, self.collectiontype_var = self.labeled_dropdown("Collection Type", values=["Art", "Books", "Coins", "Stamps", "Toys"], default="Art")

        # Description: larger text field
        self.description_text = tk.Text(self.frame, height=4, width=30)
        self.description_text.pack(fill=tk.X, padx=5, pady=5)
        tk.Label(self.frame, text="Description:").pack(anchor="w", padx=5, pady=5)

        # Submit and Cancel buttons at the bottom
        self.create_button(text="Submit", command=self.submit)
        self.create_button(text="Cancel", command=self.destroy)
    ...
    def submit(self):
        ...
        collectiontype = self.collectiontype_var.get().strip()
...
class DeleteCollectionWindow(tk.Toplevel):
    ...
    def submit(self):
        ...
        try:
            ...
            if collection:
                collection.delete()
                messagebox.showinfo("Success", f"Collection '{selected_collection}' has been deleted.")
